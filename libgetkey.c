...
static int ustring_charlen (unsigned char c) {
  if (c < 0x80) return 1;
...
utf8 getkey (int infd) {
  ...
  default:
    if (c < 0) {
      int len = ustring_charlen ((unsigned char) c);
      utf8 code = 0;
      ...
      for (idx = 0; idx < len - 1; idx++) {
        ...
        if (isnotutf8 ((unsigned char) cc)) {
          invalid = 1;
        } else {
          code <<= 6;
          code += (unsigned char) cc;
        }
      }
